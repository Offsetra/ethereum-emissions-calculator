...
export function blockData(this:any, index:number, UNIXTime:number, blockNumber:number) {
    this.index = index
    this.UNIXTime = UNIXTime
    this.blockNumber = blockNumber
}

function emissionData(this:any, UNIXTime:number, blockNumber:number, emissionFactor:number) {
    this.UNIXTime = UNIXTime
    this.blockNumber = blockNumber
    ...
    for (let i=0; i<currentBlock; i++) {
        ...
        if (i%blockResolution === 0){
            ...
            */
            ...
            let index = gasUsedArray.findIndex((x:any) => x.UnixTimeStamp === closest)

            indexArray.push(new (blockData as any)(index, gasUsed[index].UnixTimeStamp, i))
        }
    }
...
const fetchIndexesFromDayResolution = async (dayResolution:number, gas:networkData[]) => {
    ...
    for (let i=0; i<gas.length; i++){
        ...
        if (i%dayResolution === 0) {
            ...
            indexArray.push(new (blockData as any)(i, gas[i].UnixTimeStamp, blockNumber))
        }
    }
    ...
    indexArray.push(new (blockData as any)(finalIndex, finalTimestamp, finalBlock))

    return indexArray
    
}

const fetchBlockOrDayIndexArray = async (blockOrDay:string, resolution:number, currentBlock:number, gas:networkData[]) => {
    let result = []

    //Calculate using day or block range and switch function accordingly
    switch (blockOrDay) {
        case 'block':
            result = await fetchIndexesFromBlockResolution(resolution, currentBlock, gas)
            break;
        case 'day':
